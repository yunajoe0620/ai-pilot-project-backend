  ...
  async createWolframProblem(@Body() data: any) {
    ...
    try {
      ...
      if (result.ImageSrc) {
        ...
        const filePath = path.resolve('files', 'picture.png');
        console.log('filePath', filePath);

        // 파일을 저장
        fs.writeFileSync(filePath, base64, { encoding: 'base64' });

        const fileUrl = 'http://localhost:5000/files/picture.png';

        let problemprompt = `이 이미지들은 수학 그래프입니다. 이 이미지와 어울리는 문제를 만들어주세요. 난이도는 ${school}${grade}${subject}${quizSubject}여야만 한다.
        1. 문제 생성 시 필수 사항:
       - 각 문항의 수식은 MathML로 작성한다.
       - MathML 생성 시, 다음 오류를 반드시 피할 것:
         ❌ <mtable>을 <mo>, <mfenced>와 함께 사용 금지
